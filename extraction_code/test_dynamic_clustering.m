...
if exist('gts') == 0
    classes_full = zeros(size(pres_final_gt_rem, 1), 1);
    classes_full(authors_grand_topic(:, 1)) = authors_grand_topic(:, 2);
    gts = repmat(classes_full, 1, size(pres_final_gt_rem, 2)) .* pres_final_gt_rem;
end
...
for ii = 30:100
    ...
    gts(:, ii) = classes_full .* pres_final_gt_rem(:, ii);
...
%             G.SC.acc_gt = compute_accuracy(G.gt, G.SC.ass);
        end

        if (~isfield(G, 'CSC') || k ~= k_old || redo)
            fprintf('\tCompute CSC...\n');
            params = struct('d_value', round(max(20, 10*log(G.N))), 'features', 1, 'assignment', 0, 'poly_order', 100, ...
...
for ii = start_point:nb_cols
    ...
    if p > 0 && p < 0.5
        ...
        old_idx = find(pres_final_gt_rem(:, ii-1)==1);
        new_idx = find(pres_final_gt_rem(:, ii)==1);
...
%     G.dCSC.acc_gt = compute_accuracy(G.gt, G.dCSC.ass);
...
%     G.dCSC.acc_gt_full = compute_accuracy(G.gt, G.dCSC.ass_full);
...
TITLES = cell({'Accuracy', 'Kmeans', 'NCut', 'Time'});

measures = zeros(nb_cols - start_point + 1, 21); % kmcost (SC, CSC, dCSC), time (SC, CSC, dCSC)
...
figure; plot(start_point:nb_cols, measures(:, 21), '-o'); title('Evolution of rho');
